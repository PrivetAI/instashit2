...
restore_if_needed() {
  if [ "$CLEAN_START" != "true" ] && [ -f "$SNAP_FILE" ]; then
    ...
    if tar -xzf "$SNAP_FILE" -C / --no-same-owner --no-same-permissions; then
      ...
      # Восстанавливаем права для androidusr
      if id androidusr >/dev/null 2>&1; then
        # FIXED: Добавляем права на /home/androidusr/emulator
        chown -R androidusr:androidusr /home/androidusr/emulator 2>/dev/null || true
        chmod -R u+rwX,go+rX /home/androidusr/emulator 2>/dev/null || true
      ...
      log "Checking restored emulator data:"
      ls -la /home/androidusr/emulator/*.qcow2 2>/dev/null | head -5 || log "No qcow2 files found"
      ls -la /home/androidusr/.android/avd/*.ini 2>/dev/null || log "No AVD ini files found"
    else
      rc=$?
...
save_if_possible() {
  ...
  # Ждем немного чтобы эмулятор завершил запись
  log "Waiting for emulator to finish disk operations..."
  sleep 2
  ...
  if [ -z "$saver" ]; then
    log "WARNING: snapshot saver script not found"
    return 1
  fi
